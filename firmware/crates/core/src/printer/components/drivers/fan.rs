use crate::{utils::math::{Percentage, self}, printer::components::hal::pwm::PwmPin};
...
impl<P: PwmPin> Fan<P>
{
    ...
    pub fn set_speed(&mut self, mut speed: Percentage) -> Result<(), <P as PwmPin>::Error>
    {
        if speed.into_0_to_1() > 0. {
            speed = Percentage::from_0_to_1(math::map(speed.into_0_to_1(), self.minimum_duty_cycle_fan_moves.into_0_to_1()..=1., 0 as f32..=1.)).unwrap();
        }
