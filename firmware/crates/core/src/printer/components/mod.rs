...
impl<P: Peripherals> Printer3DComponents<P> {
    pub fn new(peripherals: &mut P, config: Config) -> Result<Self, CreationError>
    {
        Ok(Self
        {
            layer_fan: Fan::new(peripherals.take_layer_fan_pin().ok_or(CreationError::PeripheralMissing { name: "Layer fan"})?, config.layer_fan_min_duty_cycle_to_move),
            hotend_fan: Fan::new(peripherals.take_hotend_fan_pin().ok_or(CreationError::PeripheralMissing { name: "Hotend fan"})?, config.hotend_fan_min_duty_cycle_to_move),
        })
    }
